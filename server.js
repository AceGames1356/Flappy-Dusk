...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === '/api/progress' && req.method === 'PUT') {
      ...
      const pendingDelta = Number(user.pendingCoinDelta) || 0;
    ...
    if (req.url === '/api/admin/give-coins' && req.method === 'POST') {
      ...
      if (!target.progress) {
        target.progress = defaultProgress();
      }

      target.progress.coins = Math.max(
        0,
        (Number(target.progress.coins) || 0) + amount
      );

      target.pendingCoinDelta = (Number(target.pendingCoinDelta) || 0) + amount;

      saveDb();

      return sendJson(res, 200, {
        ok: true,
        coins: target.progress.coins
      });
    }
